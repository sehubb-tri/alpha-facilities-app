...
export const FoodSafetySummary = ({ foodSafetyChecklist }) => {
  ...
  const currentZone = FOOD_SAFETY_ZONES[checklistType];
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f3f4f6', paddingBottom: '100px' }}>
      ...
      <div style={{
        ...
        <h1 style={{ fontSize: '28px', fontWeight: '700', margin: '0 0 8px 0' }}>
          {rating === 'GREEN'
            ? FOOD_SAFETY_RAG_RULES.green.description
            : rating === 'AMBER'
            ? FOOD_SAFETY_RAG_RULES.amber.description
            : FOOD_SAFETY_RAG_RULES.red.description}
        </h1>
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '16px', fontWeight: '600', color: '#092849', marginBottom: '12px' }}>
            Service Details
          </div>
          {foodVendor && (
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ color: '#666' }}>Food Vendor:</span>
              <span style={{ fontWeight: '500' }}>{foodVendor}</span>
            </div>
          )}
          {studentParticipationCount && (
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ color: '#666' }}>Student Participation:</span>
              <span style={{ fontWeight: '500' }}>{studentParticipationCount} students</span>
            </div>
          )}
          {spotCheckTrigger && (
            <div style={{ marginTop: '12px', padding: '10px', backgroundColor: '#fef3c7', borderRadius: '6px' }}>
              <div style={{ fontSize: '13px', fontWeight: '600', color: '#92400e', marginBottom: '4px' }}>
                Spot Check Trigger:
              </div>
              <div style={{ fontSize: '14px', color: '#78350f' }}>{spotCheckTrigger}</div>
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            <span style={{
              ...
              height: '20px',
              borderRadius: '50%',
              backgroundColor: issues.length <= FOOD_SAFETY_RAG_RULES.amber.maxOpenIssues ? '#10b981' : '#ef4444',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              color: '#fff',
              ...
              {issues.length <= FOOD_SAFETY_RAG_RULES.amber.maxOpenIssues ? '✓' : '✗'}
            </span>
            <span style={{ fontSize: '14px' }}>
              3 open issues or fewer ({issues.length} total)
            </span>
          </div>
        </div>
        ...
        {instantRedIssues.length > 0 && (
          <div style={{
            ...
            <div style={{ fontSize: '13px', color: '#b91c1c', marginBottom: '12px' }}>
              These food safety issues require immediate attention.
            </div>
        ...
        {issues.length === 0 && (
          <div style={{
            ...
            <div style={{ fontSize: '14px', color: '#065f46', marginTop: '4px' }}>
              All food service quality checks passed.
            </div>
          </div>
        )}
      </div>
