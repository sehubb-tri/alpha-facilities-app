...
import { MECHANICAL_ZONES, isInstantRed, isPhotoRequired } from '../data/mechanicalZones';

export const MechanicalChecklist = ({ mechanicalChecklist, camera }) => {
  ...
  // Redirect if no checklist started
  useEffect(() => {
    if (!checklistType) {
      navigate('/mechanical');
    }
  ...
  const getSectionIcon = (sectionName) => {
    for (const [key, icon] of Object.entries(sectionIcons)) {
      if (sectionName.includes(key.split('.')[1]?.trim() || key)) {
        return icon;
      }
    }
    return '📋';
        ...
        {currentZone.sections.map((section, sectionIdx) => (
          <div key={sectionIdx} style={{ marginBottom: '24px' }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: '20px' }}>{getSectionIcon(section.name)}</span>
              {section.name}
            </div>
            {section.description && (
              <div style={{
