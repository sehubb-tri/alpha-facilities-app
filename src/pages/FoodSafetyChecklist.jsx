...
import { FOOD_SAFETY_ZONES, isInstantRed, isPhotoRequired } from '../data/foodSafetyZones';

export const FoodSafetyChecklist = ({ foodSafetyChecklist, camera }) => {
  ...
  const [localStudentCount, setLocalStudentCount] = useState('');
  ...
  useEffect(() => {
    ...
    if (studentParticipationCount) {
      setLocalStudentCount(studentParticipationCount.toString());
    }
  }, [checkResults, studentParticipationCount]);
  ...
  // Calculate progress (exclude optional checks)
  ...
  const handleStudentCountChange = (value) => {
    setLocalStudentCount(value);
    const num = parseInt(value, 10);
    if (!isNaN(num) && num >= 0) {
      setStudentParticipationCount(num);
    }
  };
                    ...
                    <div style={{
                      ...
                      {check.optional && (
                        <div style={{
                          ...
                          borderRadius: '4px',
                          marginBottom: '8px',
                          marginLeft: checkIsInstantRed ? '8px' : '0'
                        }}>
                          OPTIONAL
                        </div>
                      )}
                      ...
                      {/* Defect Checklist (for food quality) */}
                      {check.defectChecklist && (
                        <div style={{
                          ...
                          <ul style={{ margin: 0, paddingLeft: '20px', color: '#78350f' }}>
                            {check.defectChecklist.map((defect, idx) => (
                              <li key={idx} style={{ marginBottom: '4px' }}>{defect}</li>
                            ))}
                          </ul>
                        </div>
                      )}

                      {/* Number Input (for student count) */}
                      ...
                      <div style={{ display: 'flex', gap: '12px' }}>
                        ...
                        {check.optional && (
                          <button
                            onClick={() => handleAnswer(check.id, 'na', check, section.name)}
                            style={{
                              flex: 1,
                              padding: '12px',
                              borderRadius: '8px',
                              border: answer === 'na' ? '2px solid #6b7280' : '2px solid #e5e7eb',
                              backgroundColor: answer === 'na' ? '#f3f4f6' : '#fff',
                              color: answer === 'na' ? '#374151' : '#333',
                              fontWeight: '600',
                              fontSize: '16px',
                              cursor: 'pointer'
                            }}
