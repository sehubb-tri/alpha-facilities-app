...
const CHECKLIST_TYPES = [
  { id: 'monthly', name: 'Monthly Quality Check', icon: '📅', color: '#059669' },
  { id: 'spot_check', name: 'Management Spot Check', icon: '🔍', color: '#dc2626' }
];

export const FoodSafetySetup = ({ foodSafetyChecklist }) => {
  ...
  const [spotCheckTrigger, setSpotCheckTrigger] = useState('');

  const selectedZone = selectedType ? FOOD_SAFETY_ZONES[selectedType] : null;
  ...
  const handleBegin = () => {
    ...
    if (selectedType === 'spot_check' && !spotCheckTrigger.trim()) {
      alert('Please provide the reason for this spot check');
      return;
    }
    ...
    foodSafetyChecklist.initChecklist(
      campusName,
      campus,
      auditorName.trim(),
      auditorEmail.trim(),
      selectedType,
      foodVendor.trim(),
      spotCheckTrigger.trim()
    );

    window.scrollTo(0, 0);
    navigate('/food-safety/checklist');
  };

  return (
    <div className="min-h-screen bg-gray-100">
      <Header
        title="Food Service Quality"
        subtitle="Ops Quality Check"
        onBack={() => navigate('/')}
      />
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {CHECKLIST_TYPES.map((type) => {
              const zone = FOOD_SAFETY_ZONES[type.id];
        ...
        {selectedZone && (
          <div style={{
            ...
            <div style={{ fontSize: '13px', color: '#666', marginBottom: '12px' }}>
              Best done by: {selectedZone.persona}
            </div>
        ...
        {/* Spot Check Trigger (only for spot checks) */}
        {selectedType === 'spot_check' && (
          <div>
            ...
            <textarea
              placeholder="What triggered this spot check? (e.g., quality complaint, management request, etc.)"
              value={spotCheckTrigger}
              onChange={(e) => setSpotCheckTrigger(e.target.value)}
              style={{
                width: '100%',
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '12px' }}>
            ...
            <div>
              <div style={{ fontWeight: '600', color: '#059669' }}>{FOOD_SAFETY_RAG_RULES.green.description}</div>
              <div style={{ fontSize: '13px', color: '#666' }}>All checks pass, food quality excellent</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '12px' }}>
            ...
            <div>
              <div style={{ fontWeight: '600', color: '#d97706' }}>{FOOD_SAFETY_RAG_RULES.amber.description}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            ...
            <div>
              <div style={{ fontWeight: '600', color: '#dc2626' }}>{FOOD_SAFETY_RAG_RULES.red.description}</div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '14px', color: '#78350f' }}>
            Some items are marked as <strong>INSTANT RED</strong> (temperature compliance, sanitation issues). If any of these fail, the campus cannot pass until fixed.
          </div>
          <div style={{ fontSize: '14px', color: '#78350f', marginTop: '8px' }}>
            Every "No" answer requires an explanation and may require photos.
          </div>
        </div>
