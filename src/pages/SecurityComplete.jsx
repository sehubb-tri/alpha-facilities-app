...
export const SecurityComplete = ({ securityChecklist }) => {
  const navigate = useNavigate();

  const { campus, overallRating, issues } = securityChecklist;
  const instantRedCount = issues.filter(i => i.instantRed).length;

  const handleDone = () => {
    securityChecklist.resetChecklist();
    navigate('/');
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {campus} security checklist has been saved.
        </p>
          ...
          <div style={{
            ...
            {instantRedCount > 0 && (
              <div style={{ fontSize: '13px', color: '#dc2626' }}>
                • {instantRedCount} Instant RED item{instantRedCount > 1 ? 's' : ''}
              </div>
            )}
        ...
        <div style={{
          ...
          <div style={{ fontSize: '13px', color: '#1e3a8a' }}>
            {issues.length > 0
              ? 'Assign owners and fix dates to all issues within 45 days to maintain Amber status or improve to Green.'
              : 'Continue completing daily checks to maintain World Class status.'}
          </div>
        </div>
