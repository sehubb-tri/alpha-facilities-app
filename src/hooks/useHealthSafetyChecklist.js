...
import {
  ...
  isInstantRed,
  isPhotoRequired
} from '../data/healthSafetyZones';

const STORAGE_KEY = 'health_safety_checklist_state';
...
const getInitialState = () => ({
  ...
  // Which checklist type are we doing (weekly for now)
  checklistType: null,
...
export const useHealthSafetyChecklist = () => {
  const [state, setState] = useState(() => {
    ...
    try {
      ...
      console.error('Error restoring health & safety checklist state:', e);
    }
  ...
  useEffect(() => {
    if (state.startTime && !state.isComplete) {
      try {
        ...
        console.error('[useHealthSafetyChecklist] Error saving to localStorage:', e);
      }
    }
  }, [state]);
  ...
  // Initialize checklist with a specific type (weekly)
  ...
  const getCurrentZone = useCallback(() => {
    return state.checklistType ? HEALTH_SAFETY_ZONES[state.checklistType] : null;
  }, [state.checklistType]);
  ...
  const getZoneCompletionStatus = useCallback((zoneId) => {
    const zone = HEALTH_SAFETY_ZONES[zoneId || state.checklistType];
  ...
  const getChecklistData = useCallback(() => {
    ...
    const zone = HEALTH_SAFETY_ZONES[state.checklistType];
...
export default useHealthSafetyChecklist;
