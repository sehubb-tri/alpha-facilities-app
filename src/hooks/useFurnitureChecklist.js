...
import {
  ...
  isInstantRed,
  isPhotoRequired
} from '../data/furnitureZones';

const STORAGE_KEY = 'furniture_checklist_state';
...
const getInitialState = () => ({
  ...
  // Which checklist type are we doing (weekly, monthly, quarterly, annual)
  checklistType: null,
...
export const useFurnitureChecklist = () => {
  const [state, setState] = useState(() => {
    ...
    try {
      ...
      console.error('Error restoring furniture checklist state:', e);
    }
  ...
  useEffect(() => {
    if (state.startTime && !state.isComplete) {
      try {
        ...
        console.error('[useFurnitureChecklist] Error saving to localStorage:', e);
      }
    }
  }, [state]);
  ...
  // Initialize checklist with a specific type (weekly, monthly, quarterly, annual)
  ...
  const getCurrentZone = useCallback(() => {
    return state.checklistType ? FURNITURE_ZONES[state.checklistType] : null;
  }, [state.checklistType]);
  ...
  const getZoneCompletionStatus = useCallback((zoneId) => {
    const zone = FURNITURE_ZONES[zoneId || state.checklistType];
  ...
  const getChecklistData = useCallback(() => {
    ...
    const zone = FURNITURE_ZONES[state.checklistType];
...
export default useFurnitureChecklist;
